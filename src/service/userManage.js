...
class UserManageService{
  async list(user){
    ...
    let  statement=`SELECT SQL_CALC_FOUND_ROWS * FROM user WHERE \`delete\` = 0 AND registerTime BETWEEN ${start_time||0} AND ${end_time||dayjs().unix()}`
  ...
  async deleted(id){
    const statement=`UPDATE user SET \`delete\` =1 WHERE id=${id}`
    console.log(statement)
